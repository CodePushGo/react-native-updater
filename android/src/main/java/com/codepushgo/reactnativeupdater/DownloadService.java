...
public class DownloadService extends Worker {
  ...
  private static final String TAG = ReactNativeUpdaterCore.TAG;
    ...
    try {
      ...
      for (int i = 0; i < totalFiles; i++) {
        ...
        File cacheFile = new File(
          cacheFolder,
          finalFileHash + "_" + new File(fileName).getName()
        );
  ...
  private void downloadAndVerify(
    ...
    try (Response response = client.newCall(request).execute()) {
      ...
      if (!publicKey.isEmpty() && sessionKey != null && !sessionKey.isEmpty()) {
        Log.d(
          ReactNativeUpdaterCore.TAG + " DLSrv",
      ...
      if (isBrotli) {
        ...
        byte[] decompressedData = decompressBrotli(
          compressedData,
          targetFile.getName()
        );
  ...
  private byte[] decompressBrotli(byte[] data, String fileName)
    throws IOException {
    // Validate input
    if (data == null) {
    ...
    if (
      data.length == 3 && data[0] == 0x1B && data[1] == 0x00 && data[2] == 0x06
    ) {
      return new byte[0];
    }
    ...
    try (
      ...
      Log.e(
        TAG,
        "Error: Brotli process failed for " +
        fileName +
        ". Status: " +
        e.getMessage()
      );
